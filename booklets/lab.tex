...
\begin{frame}[t]{Example Scenario: Automatic Cooling}
  \small
  \begin{enumerate}
    \item Simulator reports a \textbf{high} \texttt{greenhouse\_temperature} reading.
    ...
    \[\texttt{IF greenhouse\_temperature > threshold THEN set cooling\_fan to ON}\]
...
\begin{frame}[t]{Example Scenario: Manual Cooling}
  \small
  \begin{enumerate}
     \item The operator clicks the cooling fan actuator tile on the dashboard to manually set its state to ON.
     \item The frontend sends a POST request to the API Gateway's actuator endpoint, specifying the desired state.
     \item The API Gateway forwards the actuator command to the Processor-Service via REST.
     \item The arbitrator evaluates the command alongside any active automation rules, resolving conflicts if present.
     \item Processor Service communicates with the simulator to change the actuator state.
     \item The updated actuator state is broadcast via WebSocket by the API Gateway; the dashboard reflects the new status and records the manual action in the actuator history.
  \end{enumerate}
\end{frame}

\begin{frame}[t]{Technical Architecture Recap}
  \centering
  \includegraphics[width=1\textwidth]{architecture_diagram.png}
\end{frame}
...
\begin{frame}[c]
  ...
  \footnotesize Mars Habitat -- Automation Dashboard
\end{frame}
